...
#### 6. Stack Content (Vertical Layout)

**Since:** v1.2.3 | **BEM:** `.lcms-stack`

Renders multiple content items stacked vertically with gap-based spacing. Use within columns to combine multiple content types.

**Usage:**
```php
partial('column', [
    'header' => [
        'heading' => ['title' => 'Project Progress'],
    ],
    'content' => [
        'type' => 'stack',
        'items' => [
            [
                ...
                'content' => ['html' => '<h3>Phase 1</h3><p>Complete</p>'],
            ],
            [
                'type' => 'html',
                'custom_classes' => 'lcms-card',
                'custom_id' => 'phase-2',
                'content' => ['html' => '<h3>Phase 2</h3><p>In Progress</p>'],
            ],
        ],
        'gap' => '20px',
        'align' => 'left',
...
```php
'content' => [
    'type'  => 'stack',                      // Required: specifies stack renderer
    'items' => [                             // Required: array of content items
        [
            'type'            => 'text|image|video|html|buttons|heading',
            'content'         => [...],      // Type-specific content structure
            'custom_id'       => '',         // Optional: Custom HTML id attribute
            'custom_classes'  => '',         // Optional: Additional CSS classes
            'custom_css'      => '',         // Optional: Inline styles
        ],
        // More items...
    ],
...
| `items` | array | required | Array of content items to stack |
| `gap` | string | `'20px'` | Vertical space between items |
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `type` | string | `'text'` | Content type to render |
| `content` | array | required | Content configuration for type |
| `custom_id` | string | `''` | HTML id attribute for wrapper |
| `custom_classes` | string | `''` | Space-separated CSS classes |
| `custom_css` | string | `''` | Inline CSS styles |

**Generated HTML:**
```html
<div class="lcms-stack" style="gap: 20px;">
    <div class="lcms-stack__item lcms-stack__item--html lcms-card">
        <!-- Item 1 content -->
    </div>
    <div id="phase-2" class="lcms-stack__item lcms-stack__item--html lcms-card">
        <!-- Item 2 content -->
    </div>
</div>
```
...
**Use Cases:**
- Progress trackers with multiple phases
- Cards stacked vertically with consistent spacing
- Multi-section content within a single column
- Form sections or step-by-step instructions
...
#### 7. Row Content (Horizontal Layout)

**Since:** v1.2.3 | **BEM:** `.lcms-row`

Renders multiple content items arranged horizontally in a row. Automatically stacks vertically on mobile.
...
```php
'content' => [
    'type'    => 'row',                      // Required: specifies row renderer
    'items'   => [                           // Required: array of content items
        [
            'type'            => 'text|image|video|html|buttons|heading',
            'content'         => [...],      // Type-specific content structure
            'width'           => '',         // Optional: Item width (CSS value)
            'custom_id'       => '',         // Optional: Custom HTML id attribute
            'custom_classes'  => '',         // Optional: Additional CSS classes
            'custom_css'      => '',         // Optional: Inline styles
        ],
    ...
    'align'   => 'center',                   // Vertical alignment (default: center)
    'justify' => 'start',                    // Horizontal alignment (default: start)
]
```
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `type` | string | required | Must be `'row'` |
| `items` | array | required | Array of content items for row |
| `gap` | string | `'20px'` | Horizontal space between items |
| `align` | string | `'center'` | Vertical alignment: `top\|center\|bottom` |
| `justify` | string | `'start'` | Horizontal distribution: `start\|center\|end\|space-between` |
...
| `type` | string | `'text'` | Content type to render |
| `content` | array | required | Content configuration for type |
| `width` | string | `''` | CSS width value (%, px, fr) |
| `custom_id` | string | `''` | HTML id attribute for wrapper |
| `custom_classes` | string | `''` | Space-separated CSS classes |
| `custom_css` | string | `''` | Inline CSS styles |
...
**Generated HTML:**
```html
<div class="lcms-row lcms-row--align-center lcms-row--justify-space-between" style="gap: 30px;">
    <div class="lcms-row__item lcms-row__item--image" style="flex: 0 0 40%;">
        <!-- Image content -->
    </div>
    <div class="lcms-row__item lcms-row__item--text flex flex-column gap-16" style="flex: 0 0 60%;">
        <!-- Text content -->
    </div>
</div>
```
...
- **Horizontal (`justify`)**: `start`, `center`, `end`, `space-between`

**Responsive Behavior:**
- Desktop (>768px): Items displayed horizontally
- Mobile (<768px): Items stack vertically

**Use Cases:**
- Image + text side-by-side layouts
- Icon + description combinations
- Multi-column content without grid structure
- Feature showcases

---

#### 8. Grid Content (Multi-Item Layout)

**Since:** v1.2.9 | **BEM:** `.lcms-grid-layout`

Renders a grid layout with multiple items. Supports fixed column counts or auto-responsive grids.
    ...
    'content' => [
        ...
        'items' => [
            [
                'type' => 'html',
                'custom_classes' => 'lcms-card',
                'content' => ['html' => '<h3>🎨 Design</h3><p>UI/UX services</p>'],
            ],
            [
                'type' => 'html',
                'custom_classes' => 'lcms-card',
                'content' => ['html' => '<h3>💻 Development</h3><p>Web & Mobile</p>'],
            ],
            [
                'type' => 'html',
                'custom_classes' => 'lcms-card',
                'content' => ['html' => '<h3>📱 Marketing</h3><p>Digital strategy</p>'],
            ],
        ],
        'columns' => 3,
        'gap' => '30px',
...
```php
'content' => [
    'type'      => 'grid',                   // Required: specifies grid renderer
    'items'     => [                         // Required: array of grid items
        [
            'type'            => 'text|image|video|html|buttons|heading',
            'content'         => [...],      // Type-specific content structure
            'custom_id'       => '',         // Optional: Custom HTML id attribute
    ...
    'min-width' => '250px',                  // Minimum column width (for auto modes)
    'gap'       => '30px',                   // Gap between grid items (default: 30px)
]
```
...
| `items` | array | required | Array of content items for grid |
| `columns` | int\|string | `'auto-fit'` | Column count or `'auto-fit'\|'auto-fill'` |
| `min-width` | string | `'250px'` | Minimum column width (auto modes only) |
| `gap` | string | `'30px'` | Space between grid items |
...
**Generated HTML:**
```html
<div class="lcms-grid-layout" style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 30px;">
    <div class="lcms-grid-layout__item lcms-grid-layout__item--html lcms-card">
        <!-- Item 1 -->
    </div>
    <div class="lcms-grid-layout__item lcms-grid-layout__item--html lcms-card">
        <!-- Item 2 -->
...
**Column Options:**
- **Fixed Count**: `3` → `repeat(3, 1fr)` (exactly 3 columns)
- **Auto-Fit**: `'auto-fit'` → `repeat(auto-fit, minmax(250px, 1fr))` (responsive, collapses empty)
- **Auto-Fill**: `'auto-fill'` → `repeat(auto-fill, minmax(250px, 1fr))` (responsive, keeps empty tracks)
...
**Responsive Behavior:**
- Fixed columns: Maintains column count until breakpoint
- Auto-fit/fill: Automatically adjusts based on available space
- Mobile (<768px): Typically collapses to single column
